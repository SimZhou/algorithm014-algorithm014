...
def dijkstra(Graph, start):
    ...
    while pq:
        ...
        for nextnode in Graph[node]:
            newdis = distances[node] + Graph[node][nextnode]
            if newdis < distances[nextnode] and nextnode not in visited:
                distances[nextnode] = newdis
                heapq.heappush(pq, (newdis, nextnode))
                path[nextnode] = path[node] + [nextnode]
